...
(async () => {
  try {
    ...
    // Write font files
    if (result.ttf) {
      fs.writeFileSync(path.join(outputPath, 'holicay-ui-icons.ttf'), result.ttf);
      console.log('TTF file saved.');
    }

    if (result.eot) {
      fs.writeFileSync(path.join(outputPath, 'holicay-ui-icons.eot'), result.eot);
      console.log('EOT file saved.');
    }

    if (result.woff) {
      fs.writeFileSync(path.join(outputPath, 'holicay-ui-icons.woff'), result.woff);
      console.log('WOFF file saved.');
    }

    if (result.woff2) {
      fs.writeFileSync(path.join(outputPath, 'holicay-ui-icons.woff2'), result.woff2);
      console.log('WOFF2 file saved.');
    }

    if (result.svg) {
      fs.writeFileSync(path.join(outputPath, 'holicay-ui-icons.svg'), result.svg);
